...
class Seccionista(models.Model):
    _name = 'ges.seccionista'
    _inherits = {'res.users': 'user_id'}
    _description = 'Seccionista'
    _order = 'name'
    _rec_name = 'name'

    user_id = fields.Many2one('res.users', ondelete='restrict', required=True)
    data_admissao = fields.Date('Data de admissão', required=True,
    ...
    def create(self, values):
        ...
        res = super(Seccionista, self).create(values)
        seccionistas_group = self.env.ref('gestao_equipas.ges_user_seccionista')
        seccionistas_group.write({
    ...
    def write(self, values):
        if 'email' in values:
            values['login'] = values['email']
        return super(Seccionista, self).write(values)
