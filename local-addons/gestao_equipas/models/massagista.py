...
class Massagista(models.Model):
    _name = 'ges.massagista'
    _inherits = {'res.users': 'user_id'}
    _description = 'Massagista'
    _order = 'name'
    _rec_name = 'name'
    ...
    def create(self, values):
        ...
        res = super(Massagista, self).create(values)
        massagistas_group = self.env.ref('gestao_equipas.ges_user_massagista')
        massagistas_group.write({
    ...
    def write(self, values):
        if 'email' in values:
            values['login'] = values['email']
        return super(Massagista, self).write(values)
