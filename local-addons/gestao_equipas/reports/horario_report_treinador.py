...
class HorarioReportAtleta(models.AbstractModel):
    _name = 'report.gestao_equipas.report_horario_treinador_generate'
    _inherit = ['report.gestao_equipas.report_horario_generate']

    def get_treinador(self, treinador_id):
        treinador = self.env['ges.treinador'].browse(treinador_id)
        return treinador
    ...
    def get_report_values(self, doc_ids, data=None):
        ...
        treinador = self.get_treinador(data['treinador'][0])
